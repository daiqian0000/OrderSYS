...
import com.asion.common.qrcode.QRCodeModel.Desc;
import com.asion.common.qrcode.QRCodeModel.Logo;
...
public class QRCodeServiceImpl implements QRCodeService {
	...
	public BufferedImage createQRImage(QRCodeModel model) {
		try {
			...
			BitMatrix matrix = new MultiFormatWriter().encode(model.getContent(), BarcodeFormat.QR_CODE, model.getWidth(), model.getHeight(), hints);
	...
	public BufferedImage pressDesc(BufferedImage image, Desc desc) {
		int descHeight = image.getHeight() + (desc.getFont().getSize() * desc.getTexts().length) + (desc.getFont().getSize() / 2);
	...
	public Result readQRImage(BufferedImage image) {
		...
		try {
			...
			hints.put(DecodeHintType.CHARACTER_SET, CharEncoding.UTF_8);
	...
	public String readQRImageFromFile(File file) {
		try {
			...
			if (image != null) {
				Result result = readQRImage(image);
				if (result != null) {
