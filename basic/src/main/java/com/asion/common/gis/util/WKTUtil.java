...
import com.asion.common.gis.geometry.GeometryCollection;
...
public abstract class WKTUtil {
	 ...
	 * @return 坐标系编号
	 */
	public static int getSrid(String wkt) {
		int srid = 4326;
	 ...
	 * @return 几何数据
	 */
	public static String getShape(String wkt) {
		String shape = "";
		String[] wktArr = wkt.split(";");
		if (wktArr.length >= 2) {
			shape = wktArr[1];
	...
	public static Point wktToPoint(String wkt) {
		...
		if (matcher.matches()) {
			...
			if (coords != null && coords.length >= 2) {
				return new Point(Double.valueOf(coords[0].trim()),
						Double.valueOf(coords[1].trim()), srid);
			}
		}
		return null;
	}
	 ...
	 * @return 点簇要素
	 */
	public static MultiPoint wktToMultiPoint(String wkt) {
		int srid = getSrid(wkt);
		String shape = getShape(wkt);
		String regex = ".+?\\((.+?)\\)";
		Pattern pattern = Pattern.compile(regex);
		...
		if (matcher.matches()) {
			...
			if (ArrayUtils.isNotEmpty(points)) {
				...
				for (int i = 0; i < points.length; i++) {
					...
					if (coords != null && coords.length >= 2) {
						pointArray[i] = new double[] {
								Double.valueOf(coords[0].trim()),
	 ...
	 * @return 线要素
	 */
	public static Polyline wktToPolyline(String wkt) {
		int srid = getSrid(wkt);
		String shape = getShape(wkt);
		String regex = ".+?\\((.+?)\\)";
		Pattern pattern = Pattern.compile(regex);
	 ...
	 * @return 多线要素
	 */
	public static Polyline wktToMultiPolyline(String wkt) {
		int srid = getSrid(wkt);
		String shape = getShape(wkt);
		String regex = ".+?\\(\\((.+?)\\)\\)";
		Pattern pattern = Pattern.compile(regex);
	 ...
	 * @return 面要素
	 */
	public static Polygon wktToPolygon(String wkt) {
		int srid = getSrid(wkt);
		String shape = getShape(wkt);
		String regex = ".+?\\(\\((.+?)\\)\\)";
		Pattern pattern = Pattern.compile(regex);
	 ...
	 * @return 多面要素
	 */
	public static Polygon wktToMultiPolygon(String wkt) {
		int srid = getSrid(wkt);
		String shape = getShape(wkt);
		String regex = ".+?\\(\\(\\((.+?)\\)\\)\\)";
		Pattern pattern = Pattern.compile(regex);
	 ...
	 * @return 要素集
	 */
	public static GeometryCollection wktToMultiGeometryCollection(String wkt) {
	 ...
	 * @return WKT数据
	 */
	public static String multiPointToWkt(MultiPoint multiPoint) {
	 ...
	 * @return WKT数据
	 */
	public static String polylineToWkt(Polyline polyline) {
	 ...
	 * @return WKT数据
	 */
	public static String multiPolylineToWkt(Polyline polyline) {
		if (polyline != null && ArrayUtils.isNotEmpty(polyline.getPaths())) {
				...
				if (ArrayUtils.isNotEmpty(paths[i])) {
					...
					for (int j = 0; j < paths[i].length; j++) {
						if (paths[i][j] != null && paths[i][j].length >= 2) {
							String f2 = "%s %s";
							pointArr[j] = String.format(f2, paths[i][j][0],
									paths[i][j][1]);
						}
					}
	 ...
	 * @return WKT数据
	 */
	public static String multiPolygonToWkt(Polygon polygon) {
		if (polygon != null && ArrayUtils.isNotEmpty(polygon.getRings())) {
				...
				if (ArrayUtils.isNotEmpty(rings[i])) {
					...
					for (int j = 0; j < rings[i].length; j++) {
						if (rings[i][j] != null && rings[i][j].length >= 2) {
							String f2 = "%s %s";
							pointArr[j] = String.format(f2, rings[i][j][0],
									rings[i][j][1]);
						}
					}
					pathsArr[i] = StringUtils.join(pointArr, ",");
	 ...
	 * @return WKT数据
	 */
	public static String geometryCollectionToWkt(GeometryCollection geometryCollection) {
